...
const MyOrders = ({ orders, loading, error }: MyOrdersProps) => {
  return (
            ...
            {orders.map((order) => (
              ...
              >
                ...
                <Stack spacing={0.5} sx={{ mb: 1.5 }}>
                  {order.items.map((item, idx) => (
                    <Stack key={idx} direction="row" justifyContent="space-between">
                      <Typography
                        sx={{ fontFamily: '"Inter", sans-serif', fontSize: 13.5, color: colors.fog }}
                      >
                        {item.quantity} × {item.itemName}
                      </Typography>
                      <Typography
                        sx={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 13, color: colors.fog }}
                      >
                ...
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Typography
                    sx={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 12, color: colors.fog }}
                  >
                    TOTAL
                  </Typography>
                  <Typography
                    sx={{ fontFamily: '"Space Grotesk", sans-serif', fontWeight: 600, fontSize: 16 }}
                  >
