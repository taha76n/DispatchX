...
export interface CurrentDeliveryData {
  ...
  restaurantId: { name: string; address: { text: string } };
  totalPrice: number;
}
...
const CurrentDeliveryCard = ({ delivery, onAdvance, advancing }: CurrentDeliveryCardProps) => {
  const nextLabel =
  ...
  return (
    ...
    >
      <Typography
        sx={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 11, letterSpacing: 1.5, color: colors.route, mb: 1.5 }}
      >
        CURRENT DELIVERY
      </Typography>

      <Typography sx={{ fontFamily: '"Space Grotesk", sans-serif', fontWeight: 600, fontSize: 18, mb: 0.5 }}>
      ...
      <Stack  sx={{ mb: 2, direction:"row", justifyContent:"space-between", alignItems:"center" }}>
        <Typography sx={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 12, color: colors.fog }}>
          {delivery.status === "rider_assigned" ? "Heading to pickup" : "Out for delivery"}
        </Typography>
        <Typography sx={{ fontFamily: '"Space Grotesk", sans-serif', fontWeight: 600, fontSize: 16 }}>
